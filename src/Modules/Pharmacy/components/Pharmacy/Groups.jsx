  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // Fetch the logged-in user's email from the session using the Flask API
        const response1 = await fetch('http://localhost:5000/api/get-session-email');
        const data1 = await response1.json();
  
        // Check if email is found in session
        if (response1.ok && data1.email) {
          const loggedInUserEmail = data1.email; // Get the logged-in user's email from the session
          console.log('Logged-in user email:', loggedInUserEmail);
  
          // Filter out the logged-in user from the list of verified users
          const filteredUsers = data.verified_users.filter(user => user.email !== loggedInUserEmail);
          setUsers(filteredUsers);
        } else {
          console.error('Error fetching session email:', data1.error);
        }
      } catch (error) {
            ...
            <div className="mb-4">
              ...
              <div className="overflow-y-auto max-h-60 mt-2">
                {users.map((user) => (
                  <div
                    key={user._id}
                    className={`flex items-center p-2 cursor-pointer ${selectedUsers.includes(user) ? 'bg-blue-100' : 'bg-white'}`}
